        ...
        <div class="breadcrumb-main bg-linear overflow-hidden">
            <div class="container lg:pt-[134px] pt-24 pb-10 relative">
                <div class="main-content w-full h-full flex flex-col items-center justify-center relative z-[1]">
                    ...
                    <div
                        class="filter-type menu-tab flex flex-wrap items-center justify-center gap-y-5 gap-8 lg:mt-[70px] mt-12 overflow-hidden">
                        @foreach($productTypes as $type)
                        <div class="item tab-item text-button-uppercase cursor-pointer has-line-before line-2px"
                            data-item="{{ $type['name']}}">
                            {{ $type['name'] }}
                        </div>
                        @endforeach
...
" data-item="{{$size['name']}}">
                    ...
                    <div class="filter-color pb-8 border-b border-line mt-8">
                        ...
                        <div class="list-color flex items-center flex-wrap gap-3 gap-y-4 mt-4">
                            ...
                            <div wire:click.prevent="filterByColor('{{ $color['name'] }}')"
                                class="color-item px-3 py-[5px] flex items-center justify-center gap-2 rounded-full border border-line"
                                data-item="pink">
                                <div class="color w-5 h-5 rounded-full"
                                    style="background-color:{{$color['image']}}; outline: 1px solid #eee;"
                                    data-color="{{$color['name']}}" data-item="{{$color['name']}}"></div>
                            ...
                            <div class="brand-item flex items-center justify-between" data-item="{{$brand['name']}}">
                                <div class="left flex items-center cursor-pointer">
                                    <div class="block-input">
                                        <input wire:model="productBrand" type="checkbox" name="{{$brand['name']}}"
                                            id="{{$brand['name']}}" />
                ...
                <div x-data="{ display : 'grid' }"
                    class="list-product-block style-grid lg:w-3/4 md:w-2/3 w-full md:pl-3">
                        ...
                        <div class="left flex has-line items-center flex-wrap gap-5">
                            <div class="choose-layout menu-tab flex items-center gap-2">
                                <button @click="display = 'grid' "
                                    class="item tab-item three-col p-2 border border-line rounded flex items-center justify-center cursor-pointer"
                                ...
                                <button @click="display = 'list' " :class="display === 'list' ? 'active' : ''"
                                    class="item row w-8 h-8 border border-line rounded flex items-center justify-center cursor-pointer">
                    ...
                    <div class="list-filtered flex items-center gap-3 flex-wrap">
                        ...
                        <div class="list flex items-center gap-3">
                            ...
                            <div wire:click.prevent="filterByType('')"
                                class="item flex items-center px-2 py-1 gap-1 bg-linear rounded-full capitalize"
                                data-type="type">
                        ...
                        <div class="list flex items-center gap-3">
                            <div class="w-px h-4 bg-line"></div>
                            <div wire:click.prevent="filterByColor('')"
                                class="item flex items-center px-2 py-1 gap-1 bg-linear rounded-full capitalize"
                                data-type="type">
                                <i class="ph ph-x cursor-pointer"></i>
                                <span>
                        ...
                        @endif
                        <div wire:click.prevent="resetFilters()"
                            ...
                            <div class="product-main cursor-pointer block" data-item="712">
                                <div class="product-thumb bg-white relative overflow-hidden rounded-2xl">
                                    ...
                                    <div class="product-img w-full h-full aspect-[3/4]">
                                        <img key="0" class="w-full h-full object-cover duration-700"
                                            src="{{asset('storage/'. $product->images[0]['url'])}}" alt="img">

                                        <img key="1" class="w-full h-full object-cover duration-700"
                                            src="{{asset('storage/'. $product->images[1]['url'])}}" alt="img">
                                    </div>
                                    <div
                                ...
                                <div class="product-infor mt-4 lg:mb-7">
                                    ...
                                    <div
                                        ...
                                        <div key="0" class="color-item w-8 h-8 rounded-full duration-300 relative"
                                            style="background-color:{{$color->image}}; outline: 1px solid #eee;">
                                            <div
                    ...
                    <div x-show="display === 'list'"
                        class="list-product hide-product-sold gap-8 mt-7 lg:grid-cols-3 flex flex-col" data-item="4">
                        @foreach($products as $product)
                        <div data-item="{{$product->id}}" class="product-item list-type">
                            <div
                                class="product-main cursor-pointer flex lg:items-center sm:justify-between gap-7 max-lg:gap-5">
                                <div
                                    class="product-thumb bg-white relative overflow-hidden rounded-2xl block max-sm:w-1/2">
                                    @if($product->isNew())
                                    <div
                                        class="product-tag text-button-uppercase bg-green px-3 py-0.5 inline-block rounded-full absolute top-3 left-3 z-[1]">
                                        New</div>
                                    ...
                                    <div class="list-action px-5 absolute w-full bottom-5 max-lg:hidden">
                                        <div
                                            class="quick-shop-block absolute left-5 right-5 bg-white p-5 rounded-[20px]">
                                            <div class="list-size flex items-center justify-center flex-wrap gap-2">

                                                @foreach($product->productSizes as $size)
                                                <div key="{{$size->id}}"
                                                    class="size-item w-10 h-10 rounded-full flex items-center justify-center text-button bg-white border border-line">
                                                    {{$size->name}}
                                                </div>
                                                @endforeach
                                            ...
                                            <div
                                                class="add-cart-btn button-main w-full text-center rounded-full py-3 mt-4">
                                                Add To cart</div>
                                        </div>
                                    </div>
                                </div>
                                <div
                                    class="flex sm:items-center gap-7 max-lg:gap-4 max-lg:flex-wrap lg:w-2/3 lg:flex-shrink-0 max-lg:w-full max-sm:flex-col max-sm:w-1/2">
                                    ...
                                    <div class="action w-fit flex flex-col items-center justify-center">
                                        <div
                                            class="quick-shop-btn button-main whitespace-nowrap py-2 px-9 max-lg:px-5 rounded-full bg-white text-black border border-black hover:bg-black hover:text-white">
                                            Quick Shop
                                        </div>
